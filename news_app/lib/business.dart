...
class _businessState extends State<business> {
  ...
  Future<Map<String, dynamic>> fetchData() async {
    final response = await http.get(Uri.parse(
        'https://newsapi.org/v2/everything?domains=wsj.com&apiKey=4d3aeb33c60a4a5cb5d0c3bdde6b9718'));
    ...
    return Scaffold(
        ...
        child: Column(
          children: [
            const DrawerHeader(
                ...
                child:Center
                  (child: Text('News Today...',style: TextStyle(fontSize: 30,fontWeight: FontWeight.bold,color: Colors.white),)
                ) ),
            ListTile(
              leading:const Icon(Icons.home),
              title:const Text('Home'),
              onTap:(){
                Navigator.push(context, MaterialPageRoute(builder: (context)=>MyApiCallWidget()));
              } ,
            ),
            ListTile(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: <BottomNavigationBarItem>[
          ...
          BottomNavigationBarItem(
            icon: IconButton(
                onPressed: () {}, icon: const Icon(Icons.business_sharp)),
            label: 'Business',
          )
        ],
      ),
