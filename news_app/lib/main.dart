...
import 'package:news_app/automation.dart';
...
class _MyApiCallWidgetState extends State<MyApiCallWidget> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Live News',style: TextStyle(fontWeight:FontWeight.bold,fontSize: 30 ,color: Colors.white)),
        ...
        child: Column(
          children: [
           const DrawerHeader(
               ...
               child:Center(child: Text('News Today...',style: TextStyle(fontSize: 30,fontWeight: FontWeight.bold,color: Colors.white),)
               ) ),
           ListTile(
            ...
            ListTile(
              ...
              onTap: (){
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>automation()));
              },
            ),
            ListTile(
              ...
              onTap: (){
                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>business()));
              },
            ),
            const Divider(),
            ListTile(
            ...
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String title = articles[index]['title'] ?? "No title available";
                  String author = articles[index]['author'] ?? "No author available";
                  String imageUrl = articles[index]['urlToImage'] ?? "No image available";
                  String description=articles[index]['description']??'No description available';
                  String content=articles[index]['content']??'No content available';
                  String publishedAt=articles[index]['publishedAt'];
                  return Card(
                    child: ListTile(
                        title: Text(author),
                        subtitle: Text(title,overflow: TextOverflow.ellipsis,maxLines: 3,),
                        leading: SizedBox(
                          width: 100,
                          height: 100,
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: <BottomNavigationBarItem>[
          ...
          }, icon:const Icon(Icons.mobile_screen_share_rounded)), label: 'Automobile', ),
          BottomNavigationBarItem(icon: IconButton(onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (context)=>business()));
          }, icon:const Icon(Icons.business_sharp)), label: 'Business', )
        ],
      ),
    );
  }
